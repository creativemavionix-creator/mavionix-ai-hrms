...
const SLASH_COMMANDS = [
  ...
  { cmd: "/reject-candidate", desc: "Disqualify active candidate" },
  { cmd: "/schedule-interview", desc: "Schedule an interview round" },
  { cmd: "/generate-report", desc: "Produce candidate recommendation" },
  { cmd: "/open-dashboard", desc: "Switch view to Dashboard" },
    ...
    try {
      const res = await recruiterCopilotApi.chat({
        ...
        page_context: {
          ...
          current_candidate_name: activeCandidate?.name || null,
          current_candidate_id: activeCandidate?.id || null,
  ...
  const executeProposedAction = async (msgId: string, tool: string, params: any) => {
    ...
    try {
      if (tool === "CREATE_JOB") {
        ...
        await new Promise(r => setTimeout(r, 600))
        setToolLogs(prev => [...prev, "✓ Target candidate resolved: " + params.candidate_name, "Updating application pipeline stage..."])
        // Rejects the active candidate or Priya Sharma fallback
        const targetId = activeCandidate?.id || "c1"
    ...
    <>
      {!isOpen && (
        <button
          ...
          className="fixed bottom-6 right-6 z-50 flex items-center gap-2 px-5 py-3 bg-gradient-to-r from-violet-600 to-indigo-600 hover:opacity-95 text-white text-xs font-bold uppercase tracking-wider rounded-radius-full shadow-2xl border border-violet-500/25 transition-all transform hover:scale-105 active:scale-95"
        >
      ...
      {isOpen && (
        <div 
          ...
          className="fixed right-0 top-0 bottom-0 h-screen z-50 glass-card border-l border-white/[0.08] bg-[#0a0910]/95 backdrop-blur-2xl shadow-2xl flex flex-col overflow-hidden transition-all duration-150"
        >
          {/* Resize handle bar on left boundary */}
          <div 
            onMouseDown={startResizing}
            className="absolute left-0 top-0 bottom-0 w-1.5 cursor-col-resize hover:bg-violet-500/40 transition-colors z-50"
          />
          ...
          <div className="h-16 bg-white/[0.02] dark:bg-black/40 border-b border-white/[0.05] px-5 flex items-center justify-between shrink-0">
            <div className="flex items-center gap-2.5">
              <div className="w-8 h-8 bg-violet-500/10 border border-violet-500/25 flex items-center justify-center rounded-radius-md">
                <Brain className="w-4.5 h-4.5 text-violet-400" />
              </div>
              <div>
                <h3 className="text-xs font-extrabold text-neutral-900 dark:text-white tracking-widest uppercase flex items-center gap-1.5">
                  COPILOT SYSTEM
                  <span className="text-[7.5px] bg-violet-500/15 border border-violet-500/30 text-violet-400 px-2 py-0.5 rounded-radius-full font-bold uppercase">
                    RAG-ACTIVE
                  </span>
                </h3>
          ...
          <div className="bg-white/[0.01] border-b border-white/[0.04] px-4 py-2 shrink-0 flex items-center gap-3 overflow-x-auto scrollbar-none text-[8.5px] font-bold text-neutral-400 uppercase tracking-wider">
            ...
            {activeCandidate && (
              <span className="bg-white/[0.02] border border-white/[0.08] px-2 py-0.5 rounded-radius-full text-violet-400 truncate max-w-[140px]">
            ...
            <div className="p-3 border-b border-white/[0.04] bg-white/[0.005] shrink-0">
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full bg-white/[0.01] dark:bg-black/20 border border-white/[0.08] text-[10px] text-neutral-200 pl-8 pr-3 py-1.5 rounded-radius-md outline-none placeholder:text-neutral-600 font-semibold"
                />
              </div>
            </div>
          )}
          ...
          <div className="flex-1 overflow-y-auto p-5 space-y-5 chat-scroll">
            ...
            {messages.length === 0 && dailyBrief && (
              <div className="bg-white/[0.01] dark:bg-black/20 border border-white/[0.04] p-5 space-y-4 rounded-radius-lg relative overflow-hidden">
                ...
                <div className="flex items-center justify-between border-b border-white/[0.05] pb-3 shrink-0">
                  <span className="text-xs font-bold text-violet-400 tracking-wider uppercase flex items-center gap-1.5">
                    <Sparkles className="w-4 h-4 text-violet-400" /> TODAY&apos;S RECRUITER METRIC BRIEF
                  </span>
                  <span className="text-[8px] px-2 py-0.5 bg-white/[0.04] text-neutral-400 border border-white/[0.06] rounded-radius-full font-bold uppercase tracking-wider">PROACTIVE</span>
                </div>

                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-center text-xs">
                  <div className="bg-white/[0.01] dark:bg-black/25 border border-white/[0.04] p-3 rounded-radius-md">
                    <p className="text-lg font-extrabold text-neutral-900 dark:text-white">{dailyBrief.new_applicants_today}</p>
                    <p className="text-[8px] text-neutral-400 font-bold uppercase mt-0.5 tracking-wider">NEW TODAY</p>
                  </div>
                  <div className="bg-white/[0.01] dark:bg-black/25 border border-white/[0.04] p-3 rounded-radius-md">
                    <p className="text-lg font-extrabold text-violet-400">{dailyBrief.interviews_scheduled}</p>
                    <p className="text-[8px] text-neutral-400 font-bold uppercase mt-0.5 tracking-wider">INTERVIEWS</p>
                  </div>
                  <div className="bg-amber-500/5 border border-amber-500/20 p-3 rounded-radius-md">
                    <p className="text-lg font-extrabold text-amber-400">{dailyBrief.flagged_anomalies}</p>
                    <p className="text-[8px] text-amber-400/90 font-bold uppercase mt-0.5 tracking-wider">ANOMALIES</p>
                  </div>
                  <div className="bg-emerald-500/5 border border-emerald-500/20 p-3 rounded-radius-md">
                    <p className="text-lg font-extrabold text-emerald-400">{dailyBrief.high_scorers_count}</p>
                    <p className="text-[8px] text-emerald-400/90 font-bold uppercase mt-0.5 tracking-wider">SCORE &gt; 90%</p>
                  </div>
                </div>
                  ...
                  <div className="flex-1 space-y-3 max-w-[90%]">
                    <div
                      className={`p-4 text-xs sm:text-sm leading-relaxed border rounded-radius-lg relative
                        ${isUser
                          ? "bg-violet-500/10 border-violet-500/20 text-neutral-900 dark:text-neutral-100 rounded-tr-none"
                    ...
                    >
                      ...
                      {proposal && (
                        <div className="mt-4 p-4 border border-violet-500/30 bg-violet-500/5 rounded-radius-lg space-y-3.5">
                          <div className="flex items-center gap-1.5 text-violet-400 text-[10px] font-black uppercase tracking-wider">
                            <Shield className="w-4 h-4" />
                            <span>Tool Requisition Approval</span>
                          </div>
                          
                          <div className="text-[11px] text-neutral-300 space-y-1.5">
                            <div className="flex justify-between"><span>ACTION:</span> <span className="font-extrabold text-violet-400">{proposal.tool}</span></div>
                            ...
                            {msg.proposal_executed ? (
                              ...
                              <>
                                <button
                                  ...
                                  className="flex-1 py-1.5 bg-gradient-to-r from-[#8b5cf6] to-[#d946ef] text-white text-[10px] font-bold uppercase tracking-wider rounded-radius-full transition-all hover:opacity-95"
                                >
                                  Approve Action
                                </button>
                                <button
                        ...
                        <div className="mt-4 space-y-2 pt-3 border-t border-white/[0.05]">
                          ...
                          <div className="grid grid-cols-1 gap-3">
                            {data.candidate_cards.map((cand, idx) => (
                              <div
                                key={idx}
                                className="bg-white/[0.01] dark:bg-black/25 border border-white/[0.04] p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-3 hover:border-violet-500/30 rounded-radius-lg transition-all"
                              >
                                <div className="space-y-1">
                                  <div className="flex items-center gap-2 flex-wrap">
                                    <span className="font-extrabold text-neutral-900 dark:text-white text-sm">{cand.name}</span>
                                    <span className="text-[9px] px-2 py-0.5 bg-violet-500/10 text-violet-400 border border-violet-500/25 rounded-radius-full font-bold">
                                      {cand.ai_score}% MATCH
                                    </span>
                                  </div>
                                  <p className="text-[9.5px] text-neutral-400 font-bold uppercase tracking-wider">
                                ...
                                <button
                                  ...
                                  className="px-4 py-2 bg-gradient-to-r from-violet-600 to-indigo-600 hover:opacity-95 text-white text-[10px] font-bold uppercase tracking-wider rounded-radius-full shrink-0 flex items-center justify-center gap-1.5 transition-all active:scale-[0.99] shadow-sm shadow-violet-500/10"
                                >
                                  <span>DOSSIER</span>
                              ...
                              <thead>
                                <tr className="bg-white/[0.02] dark:bg-black/25 border-b border-white/[0.05] text-neutral-400 text-[9px] font-bold uppercase tracking-wider">
                                  ...
                                  {Object.keys(data.comparison_matrix[0].values).map((name) => (
                                    <th key={name} className="p-3 text-left text-violet-400 font-extrabold text-xs">
                                      {name}
                                    </th>
                                  ))}
                                </tr>
                        ...
                        <div className="mt-4 pt-3 border-t border-white/[0.05] flex flex-wrap gap-2">
                          {data.action_buttons.map((btn, i) => (
                            <button
                              ...
                              className="px-3.5 py-1.5 bg-white/[0.02] hover:bg-violet-500/10 border border-white/[0.08] hover:border-violet-500/30 text-neutral-400 hover:text-violet-400 text-[10px] font-bold uppercase tracking-wider rounded-radius-full flex items-center gap-1 transition-all active:scale-[0.99]"
                            >
                              <span>{btn.label}</span>
          ...
          <div className="px-4 py-3.5 bg-white/[0.01] dark:bg-black/40 border-t border-white/[0.04] shrink-0 overflow-x-auto whitespace-nowrap space-x-1.5 scrollbar-none">
            {QUICK_WORKFLOW_CHIPS.map((chip, i) => (
              <button
                ...
                disabled={loading}
                className="inline-block px-3.5 py-2 bg-white/[0.02] dark:bg-black/20 hover:bg-violet-500/10 border border-white/[0.06] hover:border-violet-500/20 text-neutral-400 hover:text-violet-400 text-[10px] font-extrabold uppercase rounded-radius-full transition-all"
              >
                {chip.label}
              </button>
            ))}
          ...
          {showCommandsDropdown && (
            <div className="mx-4 mb-2 p-2 bg-[#12111d] border border-white/[0.08] rounded-radius-lg shadow-2xl space-y-1 shrink-0 max-h-36 overflow-y-auto">
              <span className="text-[8px] font-bold text-neutral-500 uppercase px-2 block tracking-widest mb-1.5">HIREMIND COPILOT SLASH ACTIONS</span>
              {SLASH_COMMANDS.map((sc, idx) => (
                ...
                >
                  <span className="font-extrabold text-violet-400">{sc.cmd}</span>
...
with open("components/recruitment/RecruiterCopilotWidget.tsx", "w", encoding="utf-8") as f:
    f.write(code)
print("Successfully redesigned frontend RecruiterCopilotWidget.tsx")
