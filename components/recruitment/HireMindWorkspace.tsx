      ...
      <aside className="hidden lg:sticky lg:top-0 lg:z-0 lg:flex h-screen w-72 shrink-0 border-r border-slate-200 dark:border-slate-800 bg-white dark:bg-[#0b0b14] flex-col">
        ...
        <div className="px-5 pt-4 pb-2">
          <div className="flex items-center gap-2.5">
            <div
              className="w-9 h-9 rounded-xl flex items-center justify-center text-white shadow-lg shadow-signal/20"
              style={{ backgroundImage: 'linear-gradient(135deg, var(--gradient-from), var(--gradient-to))' }}
            >
              <BrainCircuit size={18} />
            </div>
            <div>
              <p className="text-sm font-black leading-tight font-display text-slate-900 dark:text-white">HireMind AI</p>
              <p className="text-[11px] text-slate-500 dark:text-slate-400">MaVionix Business Suite</p>
            </div>
          </div>
        </div>
            ...
            return (
              <button
                ...
                className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-xs font-semibold transition-all text-left ${
                  isActive
                    ? "bg-signal/10 text-signal dark:bg-signal/15 dark:text-signal font-bold shadow-sm border border-signal/20"
              ...
              >
                <Icon size={16} className={isActive ? "text-signal" : "text-slate-400"} />
                <span>{tab.label}</span>
              </button>
            )
          })}
